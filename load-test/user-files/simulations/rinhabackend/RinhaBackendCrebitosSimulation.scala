...
import io.gatling.commons.validation._
import io.gatling.core.Predef._
import io.gatling.core.check.{CheckBuilder, Validator}
...
class RinhaBackendCrebitosSimulation extends Simulation {
  ...
  val validarConsistenciaSaldoLimite: (Option[String], Session) => Validation[Option[String]] = (valor, session) => {
    val saldo = valor.flatMap(s => toInt(s))
    val limite = toInt(session("limite").as[String])

    (saldo, limite) match {
      case (Some(s), Some(l)) if s < l * -1 => Failure("Limite ultrapassado!")
      case (Some(s), Some(l)) if s >= l * -1 => Success(Option("ok"))
      case _ => Failure("WTF?!")
    }
  }
  ...
  val debitos = scenario("débitos")
    .exec { s =>
      ...
      s.setAll(Map("descricao" -> descricao, "cliente_id" -> cliente_id, "payload" -> payload))
    }
    .exec(
      http("débitos")
       ...
       .check(
      status.in(200, 422).saveAs("httpStatus"),
